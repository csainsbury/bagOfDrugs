...
findSimilarDrugs <- function(inputFrame) {
  ...
  inputFrame$DrugNameNew[grep("Exenatide", inputFrame$DrugName, ignore.case = TRUE)] <- "Exenatide"
  inputFrame$DrugNameNew[grep("Eucreas", inputFrame$DrugName, ignore.case = TRUE)] <- "MetforminVildagliptin"

  inputFrame$DrugNameNew[grep("Hypurin", inputFrame$DrugName, ignore.case = TRUE)] <- "Hypurin"
  inputFrame$DrugNameNew[grep("Duloxetine", inputFrame$DrugName, ignore.case = TRUE)] <- "Duloxetine"
  ...
  inputFrame$DrugNameNew[grep("Saxagliptin", inputFrame$DrugName, ignore.case = TRUE)] <- "Saxagliptin"
  ...
  inputFrame$DrugNameNew[grep("Janumet", inputFrame$DrugName, ignore.case = TRUE)] <- "MetforminSitagliptin"
  ...
  inputFrame$DrugNameNew[grep("Humulin I", inputFrame$DrugName, ignore.case = TRUE)] <- "Humulin I"
  ...
  inputFrame$DrugNameNew[grep("Humalog Kwik", inputFrame$DrugName, ignore.case = TRUE)] <- "Humalog"
  inputFrame$DrugNameNew[grep("HUMALOG", inputFrame$DrugName, ignore.case = TRUE)] <- "Humalog"
  inputFrame$DrugNameNew[grep("Humalog Kwikpen", inputFrame$DrugName, ignore.case = TRUE)] <- "Humalog"
  
  inputFrame$DrugNameNew[grep("Competact", inputFrame$DrugName, ignore.case = TRUE)] <- "MetforminPioglitazone"
  
  inputFrame$DrugNameNew[grep("Humulin S", inputFrame$DrugName, ignore.case = TRUE)] <- "Humulin S"
...
topNdrugs_DrugNames <- subset(topNdrugs_DrugNames, Var1 != 'Bd Safe-Clip')
topNdrugs_DrugNames <- subset(topNdrugs_DrugNames, Var1 != 'Needle')
...
#      print(reportSet$drugWord)
    ...
    # function to generate drugwords for each time interval
    returnIntervals_withDirection <- function(LinkId, DrugName, prescription_dateplustime1, sequence, id) {
    ...
    for (j in seq(1, max(drugsetDT$id), )) {
      ...
      # all drugs prescribed for individual denoted j == 1
